...
function EndGameScreen({ team,iWin } : FinalScreenProps) {
    ...
    return (
        ...
        <div className="max-w-3xl mx-auto mt-10 bg-blue-900/90 text-white p-6 rounded-2xl shadow-xl border-4 border-yellow-400 text-center">
        <h1 className="text-3xl font-bold text-yellow-400 mb-4 drop-shadow">{iWin ? "Has Ganado" : "Has Perdidio"}</h1>
            ...
            <div className="flex flex-wrap justify-center gap-4">
                {team.map((poke) => (
                    <div key={poke.id} className="bg-blue-700 p-3 rounded-xl border-2 border-yellow-300 w-28 flex flex-col items-center">
                        <img
                            src={poke.sprite}
                            alt={poke.name}
